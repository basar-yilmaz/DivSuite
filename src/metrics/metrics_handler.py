...
def compute_baseline_metrics(
    ...
    Returns:
        dict: Baseline metrics including NDCG, ILD, and precomputed embeddings.
    """
    ...
    _log_baseline_metrics(
        baseline_ndcg, baseline_emb_ild, baseline_cat_ild, top_k, use_category_ild
    )
...
def _log_early_stopping_message(
    ...
    if use_category_ild:
        logger.warning(
            ...
            ndcg_drop,
            results[-2]["emb_ild"] if len(results) > 1 else baseline_metrics["emb_ild"],
            results[-2]["cat_ild"] if len(results) > 1 else baseline_metrics["cat_ild"],
        )
    else:
        logger.warning(
